...
mgr::mgr(int epollfd,const host& srv) : m_logic_srv(srv)
{
    ...
    for(int i = 0; i < srv.m_conncnt; ++i)
    {
        ...
        {
            log(LOG_INFO,__FILE__,__LINE__,"build connection %dto serve success",i);
            conn* tmp = NULL;
            try
            {
                 tmp = new conn;
...
conn* mgr::pick_conn(int cltfd)
{
    if(m_conns.empty())
    {
        log(LOG_ERR,__FILE__,__LINE__,"%s","not enough srv connections to serve");
        return NULL;
    }
    ...
    if(!tmp)
    {
        log(LOG_ERR,__FILE__,__LINE__,"%s","empty serve connection object");
        return NULL;
    }
    m_conns.erase(iter);
    ...
    log(LOG_INFO,__FILE__,__LINE__,"bind client sock %d with serve %d",cltfd,srvfd);
    return tmp;
}
        ...
        switch(type)
        {
            case READ:
            {
                ...
                switch(res)
                {
                    case OK:
                    {
			            int len = strlen(connection->m_clt_buf);
			            connection->m_clt_buf[len+1] = '\0';
                        std::cout<<connection->m_clt_buf<<"\n";
                    }
                    case BUFFER_FULL://continue this have EPOLLOUT
                    {
        ...
        switch(type)
        {
            case READ:
            {
                ...
                switch(res)
                {
                    case OK:
                    {
                        //int len = strlen(connection->m_srv_buf);
                        //connection->m_srv_buf[len+1] = '\0';
                        //std::cout<<connection->m_srv_buf<<"\n";//print OK;   
                    }
                    case BUFFER_FULL:
                    {
