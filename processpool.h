...
class processpoll
{
    ...
    public:
    static processpoll<C,H,M> * create(int listenfd,int process_number = 8)
    {
        if(!m_instance)
        {
            m_instance = new processpoll<C,H,M>(listenfd,process_number);
        }
        return m_instance;
    }
    ~processpoll()
    ...
    void run(const std::vector<H>& arg);
    ...
    static processpoll<C,H,M>* m_instance;

};
template<typename C,typename H,typename M>
processpoll<C,H,M>* processpoll<C,H,M>::m_instance = NULL;
...
template <typename C,typename H,typename M>
processpoll<C,H,M>::processpoll(int listenfd,int process_number)
...
template <typename C,typename H,typename M>
int processpoll<C,H,M>::get_most_free_srv()
{
   ...
   template <typename C,typename H, typename M>
   void processpoll<C,H,M>::setup_sig_pipe()
   {
   ...
   template <typename C,typename H,typename M>
   void processpoll<C,H,M>::run(const vector<H>& arg)
   {
       if(m_idx != -1)
       {
           run_child(arg);
   ...
   template <typename C,typename H,typename M>
   void processpoll<C,H,M>::notify_parent_busy_ratio(int pipefd,M* manager)
   {
   ...
   template <typename C,typename H,typename M>
   void processpoll<C,H,M>::run_child(const vector<H>& arg)
   {
       setup_sig_pipe();
   ...
   template <typename C,typename H,typename M>
   void processpoll<C,H,M>::run_parent()
   {
       setup_sig_pipe();
                   ...
                   else
                   {
                       for(int i = 0; i < ret; ++i)
                       {
                           switch(signals[i])
                           {
                               ...
                               case SIGINT:
                               {
                                   log(LOG_INFO,__FILE__,__LINE__,"%s","kill all child now");
