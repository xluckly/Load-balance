...
void conn::init_srv(int sockfd,const sockaddr_in& serve_addr)
{
    m_srvfd = sockfd;
    m_srv_address = serve_addr;
}
...
RET_CODE conn::read_clt()
{
    ...
    while(true)
    {
        ...
        {
            log(LOG_ERR,__FILE__,__LINE__,"%s","the client read buffers is full,let serve write");
            return BUFFER_FULL;
        }
...
RET_CODE conn::read_srv()
{
    ...
    while(true)
    {
        ...
        {
            log(LOG_ERR,__FILE__,__LINE__,"%s","the serve read buffer is full,let client write");
            return BUFFER_FULL;
        }
...
RET_CODE conn::write_srv()
{
    ...
    while(true)
    {
        ...
        if(bytes_write == -1)
        {
            ...
            log(LOG_ERR,__FILE__,__LINE__,"write serve socket failed,%s",strerror(errno));
            return IOERR;
        }
